...
public sealed class SubscriptionsDbContext : DbContext
{
    ...
    public override async Task<int> SaveChangesAsync(CancellationToken cancellationToken = new CancellationToken())
    {
        try
        {
            ...
            var outboxEvents = domainEvents.OfType<EnrollmentSuspendedEvent>().ToList();
            var directPublishEvents = domainEvents.Except(outboxEvents).ToList();
            ...
            foreach (var domainEvent in directPublishEvents)
            {
                await _serviceBus.PublishAsync(domainEvent).ConfigureAwait(false);
            }
